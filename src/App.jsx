...
async function callClaudeWithSearch(prompt, system = "") {
  ...
  for (let turn = 0; turn < 5; turn++) {
    ...
    if (system) body.system = system;
    const r = await fetch(`${SERVER_URL}/claude`, {
...
function ListStackTab({ projects, sales, setSales }) {
  ...
  const [postMode, setPostMode]           = useState("live");
  ...
  const generate = async () => {
    ...
    // Fire comps independently — never let a slow/hanging search block generation.
    const compModel = linkedProject?.model || notes.split(" ").slice(0,4).join(" ");
    setTimeout(() => { fetchComps(compModel).catch(() => {}); }, 0);
...
[3-5 sentences. Lead with what makes this item worth buying. Mention any work done as a trust signal. Be specific about condition. End with one sentence about who this is ideal for. NO bullet points. NO ALL CAPS sections.]`;
  ...
  const postToEbay = async () => {
    if (!generatedTitle || !price) {
      setPostResult({ ok: false, error: "Title and price are required before posting." });
      return;
    }
    ...
    try {
      const body = {
        ...
        sku: "GS-" + Date.now(),
        mode: postMode,
        ...(postMode === "scheduled" && scheduleTime ? { scheduleTime } : {}),
      };
      const res = await fetch(`${SERVER_URL}/ebay-list`, {
        method: "POST",
        ...
        <>
          ...
          {showPreview&&generatedTitle&&(
            <div style={{marginBottom:14}}>
              ...
              <div className="box" style={{marginBottom:8}}>
                <div className="sl" style={{marginBottom:6}}>Post to eBay</div>
                <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:8}}>
                  {["live","scheduled","draft"].map(m=>(
                    <button key={m} className={`pill${postMode===m?" on":""}`} onClick={()=>setPostMode(m)} style={{textTransform:"capitalize"}}>
                      {m==="live"?"🟢 List Live":m==="scheduled"?"🕐 Schedule":"💾 Save Draft"}
                    </button>
                  ))}
                </div>
                {postMode==="scheduled"&&(
                ...
                <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
                  <button className="btn bp" style={{flex:1}} onClick={postToEbay} disabled={posting}>
                    {posting?"Posting…":postMode==="live"?"Post to eBay Now":postMode==="scheduled"?"Schedule Listing":"Save as Draft"}
                  </button>
                  <button className="btn bs" onClick={markPosted}>Save to History + P&L</button>
                  <button className="btn bs" onClick={()=>copy(generatedTitle+"\n\n"+generatedDesc)}>{copied?"Copied!":"Copy"}</button>
                  <button className="btn bs" onClick={generate} disabled={generating}>Regen</button>
                </div>
                {postResult&&(
                  <div style={{marginTop:8,fontSize:11,padding:"7px 9px",borderRadius:4,background:postResult.ok?"rgba(71,255,178,.07)":"rgba(255,71,87,.07)",border:`1px solid ${postResult.ok?"rgba(71,255,178,.2)":"rgba(255,71,87,.2)"}`,color:postResult.ok?"var(--success)":"var(--danger)"}}>
                    {postResult.ok?"✓ "+postResult.message:"✗ "+postResult.error}
                  </div>
                )}
              </div>
